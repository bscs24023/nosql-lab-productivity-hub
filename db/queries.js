...
async function createProject(db, projectData) {
  const result = db.collection('projects').insertOne({
...
async function listProjectTasks(db, projectId, status) {
  // TODO: implement
  const filtered_tasks = { projectId: projectId };
  if(status)
  {
    filtered_tasks.status = status;
  }
...
async function searchNotes(db, ownerId, tags, projectId) {
  ...
  if(projectId)
  {
    filter_notes.projectId = projectId;
  }
  return await db.collection('notes').find(filter_notes).sort({ createdAt: -1 }).toArray();
}
